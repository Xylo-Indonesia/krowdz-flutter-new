...
abstract class _VisitorListStore with Store {
  /* rest of the class*/
  @observable KeyMap selectedKey,selectedKey2;
  @observable String keyword='';

  @observable Visitor visitor;
  @observable bool isVisitorReady=false;

  @observable List<KeyMap> items;
  @observable List<KeyMap> items2;

  @observable Map<String, dynamic> dyn_visitor;

  @observable String page="1";
  @observable String order_by="created_at";


  //pagination
  @observable bool hasNext=true;
  @observable bool hasPrev=true;
  @observable bool hasPagination=true;

  void getVisitorList({String keyword='',String keyword_by='',String page='',String order_direction=''}) async{
    isVisitorReady=false;
    var response= await API.VisitorList(keyword:keyword,keyword_by: keyword_by,page:page,order_direction: order_direction);
    print(response);
    ...
    if(null==selectedKey) {
      ...
      dyn_visitor["meta"]["category"].forEach((k,v){
        ...
        if(null!=keyword_by&& keyword_by.length>0){
          if(keyword_by==k.toString())
            selectedKey=items.last;
        }else{
  ...
  void doNextPage(){
    if(hasNext){
      getVisitorList(keyword: keyword,keyword_by: selectedKey.key,page:visitor.links.next);
    }
  }
  void doPrevPage(){
    if(hasPrev){
      getVisitorList(keyword: keyword,keyword_by: selectedKey.key,page:visitor.links.prev);
    }
  }
